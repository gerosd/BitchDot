...
export default function Header({categories, shouldLoadFromDB}: { categories: Category[]; shouldLoadFromDB: boolean }) {
        ...
        <header className="sticky top-0 z-50 w-full border-b border-border bg-base shadow-sm">
            <Container>
                ...
                <nav className="hidden md:flex h-14 items-center justify-center gap-10 lg:gap-14 relative">
                    <Link href="/about"
                          className="text-[13px] font-medium tracking-wide text-gray-800 hover:text-black hover:opacity-70 transition-opacity uppercase">О
                        бренде</Link>
                            ...
                            {isCatalogOpen && (
                                ...
                                >
                                    <div className="w-1/3 bg-gray-50/50 py-4 border-r border-gray-100">
                                        {shouldLoadFromDB ? categories.map(category => (
                                                <button
                                                ...
                                                >
                                                    {category.name}
                                                    <ChevronRight
                                                        className={`h-4 w-4 text-gray-400 transition-opacity ${activeCategory?._id === category._id ? 'opacity-100' : 'opacity-0'}`}
                                                        strokeWidth={2}/>
                                                </button>
                                            ))
                                            :
                                                <h1 className="py-24 text-center min-h-[10vh] text-xl font-bold mb-4">Сайт находится на обслуживании</h1>
                                        }
                                    </div>
                                    {shouldLoadFromDB && activeCategory ? (
                                        <div className="w-2/3 p-8 bg-white min-h-75 relative">
                                            ...
                                            <div
                                                className="mt-8 pt-6 border-t border-gray-100 absolute bottom-8 w-[calc(100%-4rem)]">
                                                <Link
                                                    ...
                                                    onClick={() => setIsCatalogOpen(false)}
                                                >
                                                    Смотреть все из раздела {activeCategory.name.toLowerCase()}
                                                    <ChevronRight className="h-3 w-3"/>
                                                </Link>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="w-2/3 p-8 bg-white min-h-75">
                                            <p className="text-gray-500">В данный момент каталог недоступен.</p>
                                        </div>
                                    )}
                                </motion.div>
                            )}
                    ...
                    <Link href="/reviews"
                          className="text-[13px] font-medium tracking-wide text-gray-800 hover:text-black hover:opacity-70 transition-opacity uppercase">Обзоры
                        и рекомендации</Link>
                    <Link href="/where-to-buy"
                          className="text-[13px] font-medium tracking-wide text-gray-800 hover:text-black hover:opacity-70 transition-opacity uppercase">Где
                        купить</Link>
                </nav>
            </Container>
                ...
                {isMobileMenuOpen && (
                    ...
                    >
                        <div className="flex h-18 items-center justify-between px-4 border-b border-border">
                            <Link href="/" className="text-2xl font-serif tracking-[0.2em] text-base-content"
                        ...
                        <nav className="p-6 flex flex-col gap-6">
                            <form onSubmit={handleSearch}
                                  className="pb-6 border-b border-border-light flex items-center justify-between group">
                                <input
                                    type="text"
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                ...
                                <button type="submit" className="p-2 -mr-2">
                                    <Search
                                        className="h-5 w-5 text-surface-content-muted group-hover:text-primary transition-colors"
                                        strokeWidth={1.5}/>
                                </button>
                            </form>
                            <Link href="/catalog"
                                  className="text-lg font-medium text-base-content uppercase tracking-widest"
                                  onClick={() => setIsMobileMenuOpen(false)}>Каталог</Link>
                            <Link href="/catalog?sort=newest"
                                  className="text-lg font-medium text-base-content uppercase tracking-widest"
                                  onClick={() => setIsMobileMenuOpen(false)}>Новинки</Link>
                            <Link href="/about"
                                  className="text-lg font-medium text-base-content uppercase tracking-widest"
                                  onClick={() => setIsMobileMenuOpen(false)}>О бренде</Link>
                            <Link href="/where-to-buy"
                                  className="text-lg font-medium text-base-content uppercase tracking-widest"
                                  onClick={() => setIsMobileMenuOpen(false)}>Где купить</Link>

                            <div className="mt-8 pt-8 border-t border-border flex flex-col gap-4">
                                <Link href="/admin"
                                      className="text-surface-content hover:text-primary transition-colors flex items-center gap-3"
                                      onClick={() => setIsMobileMenuOpen(false)}>
